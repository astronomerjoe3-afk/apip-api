...
F4_MODULE_ID = "F4"
F4_CONTENT_VERSION = "20260313_f4_circuits_v1"
F4_ALLOWLIST = [
...
F4_MODULE_DOC, _LESSONS, _SIMS = build_nextgen_module_scaffold(
    ...
    "Build on F3 by treating circuits as systems for charge flow and energy transfer, not as disconnected formulas.",
    [
...
configure_sim("F4_L1", "f4_charge_current_lab", "Charge and Current Explorer", "Track charge flow in a closed loop so current is seen as a rate and not as something a component uses up.", [
...
configure_lesson(
    "F4_L1",
    "A bead chain in a loop helps: the bead rate past a point is current, and the same beads keep circulating even when energy is transferred.",
    "Predict what stays the same and what can change when the same charge passes different points in one closed loop.",
    [prompt_block("Compare charge with current.", "Charge is an amount; current is charge per second."), prompt_block("Compare current before and after a lamp.", "The lamp transfers energy but does not use current up.")],
    [
    ...
    ],
    {
        ...
        "analogy_map": {"comparison": "A bead chain loop stands for a closed circuit.", "mapping": ["Beads represent charge carriers.", "Bead rate past a marker represents current."], "limit": "Real energy transfer is not visible beads carrying all the energy.", "prediction_prompt": "If the chain moves faster, what happens to the bead rate everywhere?"},
...
configure_sim("F4_L2", "f4_potential_difference_lab", "Potential Difference Explorer", "Compare cells and components so potential difference is seen as energy transferred per charge.", [
...
configure_lesson(
    "F4_L2",
    "Potential difference is like how much energy each packet gets from a lift: the packet count can stay the same while the energy per packet changes.",
    "Predict whether the question is about total charge or energy transferred per coulomb.",
    [prompt_block("Compare two cells moving the same charge.", "More volts means more energy per coulomb."), prompt_block("Compare the role of a cell with the role of a resistor.", "A cell gives charge electrical energy; a component transfers it away.")],
    [
    ...
    ],
    {
        ...
        "analogy_map": {"comparison": "A lift giving height to each packet stands for a cell giving energy to each coulomb.", "mapping": ["Packets represent moving charge.", "Height per packet represents energy per charge."], "limit": "Charges are not literally lifted upward in the circuit.", "prediction_prompt": "If the lift is taller for the same packet count, what changes?"},
...
configure_sim("F4_L3", "f4_resistance_iv_lab", "Resistance and I-V Explorer", "Vary voltage and resistance together so current and graph steepness are interpreted as linked relationships.", ["Keep resistance fixed and increase voltage.", "Keep voltage fixed and compare low and high resistance.", "Compare two straight I-V lines with different steepness."], ["resistance_current_relationship_confusion", "iv_graph_reasoning_confusion"], ["voltage_changes", "resistance_changes", "graph_comparisons", "ohmic_explanations"], "number of voltage-resistance comparisons linked to a correct I-V explanation")

configure_lesson(
    "F4_L3",
    "Resistance is like a narrow doorway: for the same push, a narrower doorway lets fewer people through each second.",
...
configure_sim("F4_L6", "f4_power_safety_lab", "Electrical Power and Safety Explorer", "Compare voltage, current, time, and protection devices so power, energy, and safety are reasoned together.", ["Hold voltage fixed and raise the current.", "Hold power fixed and increase the running time.", "Compare two devices and judge which needs the higher protection rating."], ["power_energy_circuit_confusion", "electrical_safety_confusion"], ["voltage_changes", "current_changes", "time_changes", "safety_explanations"], "number of power-energy comparisons linked to a correct current-safety explanation")

configure_lesson(
    "F4_L6",
    "Electrical power is like how quickly a pump delivers energy to a system: the rate depends on how much energy moves each second, and safety devices interrupt the process when the current becomes dangerously large.",
    "Decide whether the question is about rate of transfer, total energy used over time, or why a protection device must break the circuit.",
    [prompt_block("Compare two devices on the same voltage but with different currents.", "At the same voltage, the device with the larger current has the greater power."), prompt_block("Compare the same device running briefly and for a long time.", "Power is a rate, so total energy also depends on time.")],
    ...
    "Electrical power is the rate of energy transfer in a circuit, and safety devices interrupt the circuit when current becomes large enough to cause dangerous heating.",
