...
def lesson_l1() -> Dict[str, Any]:
    return {
        ...
        "transfer": [
            ...
            short("M4L1_M6", "Why do flat training shoes mark the floor less than spikes under the same athlete?", ["because the same force is spread over a larger area", "because the force is shared over more patches", "because the pressure is lower when the area is larger"], "Use same-force-larger-area language.", ["pressure_area_confusion"], acceptance_rules=acceptance_groups(["larger", "more", "wider", "bigger", "broader"], ["area", "contact area", "patch", "spread", "footprint"], ["lower", "less", "smaller"], ["pressure", "mark", "damage", "load"])),
...
def lesson_l2() -> Dict[str, Any]:
    return {
        ...
        "transfer": [
            ...
            short("M4L2_M5", "Why does doubling the push require doubling the area when the safe pressure limit is unchanged?", ["because area must rise in step with force to keep the same pressure", "because pressure equals force divided by area", "because the ratio must stay constant"], "Keep the fixed-limit idea visible.", ["pressure_area_confusion"], acceptance_rules=acceptance_groups(["force", "push"], ["area", "footprint", "contact area"], ["same", "keep", "constant", "fixed", "unchanged"], ["pressure", "limit", "ratio"])),
...
def lesson_l3() -> Dict[str, Any]:
    return {
        ...
        "recon_prompts": [
            ...
            "The formal summary p = ρgh compresses the layer-stack story.",
        ],
        ...
        "capsule_checks": [
            ...
            mcq("M4L3_C2", "Which formal relation matches the liquid layer-stack story?", ["P = F/A", "p = ρgh", "E = Pt", "F = ma"], 1, "Depth, density, and g belong here.", ["liquid_density_confusion"]),
            short("M4L3_C3", "Why does a denser liquid give greater pressure at the same depth?", ["because each layer is heavier", "because density makes the liquid stack heavier", "because ρ is larger in ρgh"], "Use heavier-layer language.", ["liquid_density_confusion"], acceptance_rules=acceptance_groups(["denser", "density", "rho", "ρ"], ["heavier", "more weight", "greater weight", "more mass"], ["layer", "stack", "same depth", "same level", "same height"])),
            ...
            mcq("M4L3_C5", "What does g represent in p = ρgh here?", ["charge", "World Pull", "time", "volume"], 1, "It is the world's pull on the liquid layers.", ["liquid_density_confusion"]),
        ...
        "diagnostic": [
            ...
            short("M4L3_D3", "Water has density 1000 kg/m^3. At depth 3 m with g = 10 N/kg, what hydrostatic pressure is produced?", ["30000 Pa", "30000"], "Use p = ρgh.", ["liquid_density_confusion"], acceptance_rules=acceptance_groups(["30000"])),
            ...
            short("M4L3_D5", "What three factors set liquid pressure in p = ρgh?", ["density, gravitational field strength, and depth", "ρ, g, and h", "liquid density, world pull, and depth"], "Name density, g, and depth.", ["liquid_density_confusion"], acceptance_rules=acceptance_groups(["density", "rho", "ρ"], ["g", "world pull", "gravitational", "gravity", "field strength"], ["depth", "height", "h", "below the surface"])),
        ...
        "transfer": [
            mcq("M4L3_M1", "At what depth in water (ρ = 1000 kg/m^3, g = 10 N/kg) is the hydrostatic pressure 50000 Pa?", ["2 m", "5 m", "10 m", "50 m"], 1, "Use h = p / ρg.", ["liquid_density_confusion"]),
            short("M4L3_M2", "A liquid has density 800 kg/m^3. At depth 4 m with g = 10 N/kg, what pressure is produced?", ["32000 Pa", "32000"], "Use p = ρgh.", ["liquid_density_confusion"], acceptance_rules=acceptance_groups(["32000"])),
            ...
            mcq("M4L3_M4", "What should stay visible before using p = ρgh?", ["the layer-stack story at that location", "the color of the tank", "the total amount of liquid everywhere", "the force on only the bottom"], 0, "Pressure belongs to a location in the liquid.", ["same_depth_shape_confusion"]),
            short("M4L3_M5", "Why does doubling depth double liquid pressure when density and g stay fixed?", ["because pressure is directly proportional to depth", "because there are twice as many layers above", "because h doubles in ρgh"], "Use direct-proportion or layer-stack language.", ["liquid_density_confusion"], acceptance_rules=acceptance_groups(["depth", "h"], ["double", "twice", "direct", "proportional"], ["layer", "stack", "above", "rhogh", "ρgh", "pressure"])),
        ...
        "contract": contract(
            concept_targets=["Liquid pressure as ρgh", "Depth dependence", "Density and g effects"],
            prerequisite_lessons=["M4_L1"],
            misconception_focus=["same_depth_shape_confusion", "liquid_density_confusion"],
            formulas=[formula("p = ρgh", "Hydrostatic pressure from liquid density, World Pull, and depth", ["Pa", "kg/m^3", "N/kg", "m"], "Use for a resting liquid of roughly constant density.")],
            representations=[
            ...
            worked_examples=[
                worked("Water has density 1000 kg/m^3. Find the hydrostatic pressure at depth 4 m when g = 10 N/kg.", ["Identify a resting-liquid pressure story, so use p = ρgh.", "Substitute 1000 x 10 x 4.", "State the result as pressure at that depth."], "The hydrostatic pressure is 40000 Pa.", "At 4 m depth, the water stack gives 1000 x 10 x 4 = 40000 Pa of pressure.", "This is the direct quantitative form of the layer-stack idea."),
                worked("Two windows are at the same 3 m depth, one in water and one in oil of density 800 kg/m^3. Which feels greater pressure?", ["Keep depth and g the same across the comparison.", "Use density as the deciding factor in p = ρgh.", "Compare 1000 x 10 x 3 with 800 x 10 x 3."], "The water window feels greater pressure.", "At the same depth and same g, the denser liquid gives the greater pressure because its layers are heavier.", "This keeps density visible instead of letting depth do all the explanatory work."),
                worked("A liquid patch feels 60000 Pa on a world where g = 10 N/kg. The liquid density is 1200 kg/m^3. How deep is the patch?", ["Use p = ρgh with depth as the unknown.", "Rearrange to h = p / (ρg).", "Substitute 60000 / (1200 x 10)."], "The depth is 5 m.", "60000 divided by 12000 gives 5, so the patch is 5 m below the surface.", "This inverse problem shows that the same relation also solves for depth when the pressure is known."),
            ],
            ...
            simulation_contract={
                ...
                "takeaway": "Liquid pressure grows with ρ, g, and h because deeper patches carry heavier layer stacks.",
            },
            ...
            mastery_skills=["Use p = ρgh", "Explain depth and density effects", "Solve direct and inverse hydrostatic problems"],
            variation_plan={
            ...
            scaffold_support=scaffold(
                ...
                "Name the location first, then ask how much liquid is above it, how heavy that liquid is, and how strongly the world pulls the layers down. After that, use p = ρgh.",
                "If two patches are in the same liquid but one is deeper, what extra story does the deeper patch have above it?",
                "Do not let tank shape replace depth in your explanation. Point pressure in a resting liquid depends on ρ, g, and h.",
                ...
                extra_sections=[
                    ...
                    extra_section("Three-factor thinking", "p = ρgh is stronger than a height-only story because density and World Pull still matter when depth is unchanged."),
                ],
            ),
        ),
    }
...
def lesson_l4() -> Dict[str, Any]:
    return {
        ...
        "capsule_checks": [
            ...
            mcq("M4L4_C7", "Both vessels hold water with ρ = 1000 kg/m^3 and g = 10 N/kg. Two windows are each 2.5 m below the surface, one in a narrow tank and one in a wide tank. What pressure does each window read?", ["2500 Pa", "25000 Pa", "50000 Pa", "different pressures because the vessel shapes differ"], 1, "Use p = ρgh after noticing the same liquid and same depth.", ["same_depth_shape_confusion"]),
        ],
        "diagnostic": [
            ...
            short("M4L4_D3", "Two windows are both 2 m below the surface in the same water tank. What can you say about their pressures?", ["they are equal", "same pressure", "equal pressure"], "Use equal-depth language.", ["same_depth_shape_confusion"], acceptance_rules=acceptance_groups(["equal", "same", "match", "unchanged", "identical"], ["pressure", "pressures", "reading", "meter"])),
            ...
            short("M4L4_D9", "A window in a wide water tank and a window in a narrow water tube are both 3 m below the surface. If ρ = 1000 kg/m^3 and g = 10 N/kg, what pressure does each window read?", ["30000 Pa", "30000"], "Use p = ρgh, then apply the same answer to both because the liquid and depth match.", ["same_depth_shape_confusion"], acceptance_rules=acceptance_groups(["30000"])),
        ],
        "transfer": [
            ...
            short("M4L4_M2", "Why does equal depth beat vessel shape in a same-liquid comparison?", ["Equal depth in the same liquid gives the same pressure, so vessel shape does not matter.", "Pressure is set by the local depth in that liquid, not by container shape.", "The liquid stack above each point matches, so the pressure matches."], "Explain that the same liquid and the same depth fix the pressure before shape enters the story.", ["same_depth_shape_confusion"], acceptance_rules=acceptance_groups(["depth", "same depth", "equal depth", "level", "same level", "location", "point", "local"], ["pressure", "patch load", "reading", "meter"], ["same", "equal", "match", "set", "depends", "determines", "fixed"], ["shape", "container", "vessel", "tank"])) ,
            ...
            mcq("M4L4_M4", "At one same-depth location in a resting liquid, what is not needed to determine the pressure?", ["density", "g", "depth", "tank width"], 3, "Width is not part of p = ρgh.", ["same_depth_shape_confusion"]),
            short("M4L4_M5", "A water patch at 3 m depth has pressure p. Another water patch at the same depth in a different-shaped container has what pressure?", ["The same pressure p.", "It has the same pressure.", "Still p, because the depth and liquid match.", "p"], "State that the pressure stays p because the liquid and depth are unchanged.", ["same_depth_shape_confusion"], acceptance_rules=acceptance_groups(["same", "equal", "match", "unchanged", "identical", "still", "also"], ["pressure", "reading", "meter", "value", "p"])) ,
            ...
            mcq("M4L4_M9", "Window A is 2 m below the water surface in a narrow vessel. Window B is 2 m below the water surface in a wide vessel. Water has ρ = 1000 kg/m^3 and g = 10 N/kg. Which comparison is correct?", ["A = B = 20000 Pa", "A > B because the narrow vessel crowds the liquid more", "B > A because the wide vessel holds more water", "The pressure cannot be compared without the window areas"], 0, "Same liquid and same depth give equal pressure, and p = ρgh gives 20000 Pa for both.", ["same_depth_shape_confusion"]),
        ],
        "contract": contract(
            ...
            formulas=[formula("p = ρgh", "Hydrostatic pressure at a point", ["Pa", "kg/m^3", "N/kg", "m"], "Use the same liquid and same depth rule to compare points before calculating.")],
            representations=[
...
def lesson_l5() -> Dict[str, Any]:
    return {
        ...
        "diagnostic": [
            ...
            short("M4L5_D5", "Why does a larger patch feel a larger force at the same pressure?", ["because force equals pressure times area", "because more area collects the same pressure over more surface", "because F = pA"], "Use pressure-times-area language.", ["total_pressure_confusion"], acceptance_rules=acceptance_groups(["pressure", "p"], ["area", "surface", "patch"], ["force", "f"], ["times", "multiplies", "more surface", "same pressure"])),
        ...
        "transfer": [
            ...
            short("M4L5_M2", "Why can fluid push on a side wall even though the liquid is not 'falling sideways'?", ["Pressure exists at the fluid location, and the force on the wall acts perpendicular to the wall.", "Pressure is a scalar at the point, so the wall sets the force direction.", "The liquid does not need to flow sideways for pressure to push normal to the wall."], "Say that the pressure is there at the point and the wall force is normal to the wall.", ["fluid_pressure_direction_confusion"], acceptance_rules=acceptance_groups(["pressure", "scalar", "location", "point", "fluid"], ["wall", "side", "surface", "patch"], ["perpendicular", "normal", "force", "push", "direction"])) ,
            ...
            short("M4L5_M5", "A patch force is 1800 N and the pressure is 9000 Pa. What is the patch area?", ["0.20 m^2", "0.2 m^2", "0.20 square metres", "0.2 square metres", "0.20 square meters", "0.2 square meters", "0.200 m^2", "2e-1 m^2", "0.2", "0.20"], "Use A = F / p, so 1800/9000 = 0.20 m^2.", ["total_pressure_confusion"], acceptance_rules=acceptance_groups(["0.2", "0.20", "0.200", "2e-1"])) ,
            ...
            mcq("M4L5_M8", "Why is this lesson a step beyond just p = ρgh?", ["It adds the distinction between scalar pressure and force on a chosen surface", "It removes area from the story", "It says pressure has no relation to force", "It turns pressure into momentum"], 0, "This lesson links point pressure to surface force correctly.", ["fluid_pressure_direction_confusion"]),
        ],
        "contract": contract(
...
def lesson_l6() -> Dict[str, Any]:
    return {
        ...
        "micro_prompts": [
            ...
            prompt_block("What does p_total = p_atm + ρgh mean physically?", "Add sky blanket and liquid stack."),
        ...
        "inquiry": [
            ...
            prompt_block("Now add liquid depth below an open surface. What extra contribution appears?", "The liquid stack adds ρgh."),
        ...
        "recon_prompts": [
            ...
            "Pressure below the surface of an open liquid can be written as p_total = p_atm + ρgh.",
        ...
        "capsule_checks": [
            ...
            mcq("M4L6_C2", "What does p_total = p_atm + ρgh mean in words?", ["total pressure equals sky-blanket load plus liquid-stack load", "pressure equals force plus energy", "liquid pressure replaces atmospheric pressure", "air pressure is zero in liquids"], 0, "Add atmosphere and liquid contributions.", ["total_pressure_confusion"]),
            ...
            mcq("M4L6_C4", "At a fixed liquid depth, what happens to total pressure if atmospheric pressure drops?", ["it rises", "it falls", "it stays the same", "it becomes only ρgh automatically"], 1, "Total pressure includes the atmospheric part.", ["total_pressure_confusion"]),
        ...
        "diagnostic": [
            ...
            mcq("M4L6_D2", "What is the total pressure 2 m below the surface of water if p_atm = 100000 Pa, ρ = 1000 kg/m^3, and g = 10 N/kg?", ["20000 Pa", "100000 Pa", "120000 Pa", "140000 Pa"], 2, "Add p_atm + ρgh.", ["total_pressure_confusion"]),
            short("M4L6_D3", "Water is open to the atmosphere. At depth 5 m, with p_atm = 100000 Pa, ρ = 1000 kg/m^3, and g = 10 N/kg, what is the total pressure?", ["150000 Pa", "150000"], "Add atmospheric and hydrostatic pressure.", ["total_pressure_confusion"], acceptance_rules=acceptance_groups(["150000"])),
        ...
        "transfer": [
            ...
            short("M4L6_M2", "At depth 3 m in water, with p_atm = 100000 Pa, what is the total pressure if ρ = 1000 kg/m^3 and g = 10 N/kg?", ["130000 Pa", "130000", "130 kPa", "130kPa", "1.3e5 Pa", "1.30e5 Pa"], "Use p_total = p_atm + ρgh, so 100000 Pa + 30000 Pa = 130000 Pa.", ["total_pressure_confusion"], acceptance_rules=acceptance_groups(["130000", "130 kPa", "130kPa", "1.3e5", "1.30e5"])) ,
            mcq("M4L6_M3", "Which quantity stays the same when moving from sea level to a mountain in the same liquid tank if depth and liquid are unchanged?", ["atmospheric pressure", "ρgh", "total pressure", "air above"], 1, "The liquid stack part can stay the same while the atmospheric part changes.", ["total_pressure_confusion"]),
            ...
            short("M4L6_M5", "Why does the atmosphere matter in an open liquid pressure problem?", ["Because atmospheric pressure still presses on the open surface, total pressure is atmosphere plus liquid pressure.", "Because p_total includes atmospheric pressure as well as ρgh.", "Because the sky blanket adds to the liquid stack in an open liquid."], "Say that an open surface still has the atmosphere above it, so it must be added to the total.", ["total_pressure_confusion"], acceptance_rules=acceptance_groups(["atmosphere", "atmospheric", "air", "sky blanket", "sky"], ["add", "plus", "part of", "included", "contributes", "counts"], ["total", "p_total", "pressure", "open liquid", "open surface", "surface"])) ,
            ...
            mcq("M4L6_M7", "A liquid patch is open to the atmosphere. Which expression gives the pressure due only to the liquid layers, not the total?", ["p_atm", "ρgh", "p_atm + ρgh", "F/A only"], 1, "ρgh is the hydrostatic contribution only.", ["total_pressure_confusion"]),
        ...
        "contract": contract(
            ...
            formulas=[formula("p_total = p_atm + ρgh", "Total pressure below an open liquid surface", ["Pa", "Pa", "kg/m^3", "N/kg", "m"], "Use when atmospheric pressure above the liquid surface matters.")],
            representations=[
            ...
            worked_examples=[
                worked("Find the total pressure 2 m below the surface of water when p_atm = 100000 Pa, ρ = 1000 kg/m^3, and g = 10 N/kg.", ["Identify an open-surface liquid problem, so total pressure includes atmosphere and liquid.", "Find the liquid part: ρgh = 1000 x 10 x 2 = 20000 Pa.", "Add atmospheric pressure: 100000 + 20000."], "The total pressure is 120000 Pa.", "Open-surface total pressure is atmospheric pressure plus hydrostatic pressure, so 100000 Pa + 20000 Pa = 120000 Pa.", "This is the module's capstone pressure sum: sky blanket plus liquid stack."),
            ...
            simulation_contract={
                ...
                "takeaway": "Air is also a fluid. Low altitude means more sky blanket, and open-liquid total pressure adds that sky load to ρgh.",
            },
            reflection_prompts=["Why is atmospheric pressure lower higher up?", "Why is total pressure below an open liquid surface larger than ρgh alone?"],
            mastery_skills=["Explain atmospheric pressure conceptually", "Use p_total = p_atm + ρgh", "Solve mixed air-plus-liquid pressure problems"],
            variation_plan={
            ...
            scaffold_support=scaffold(
                ...
                "Decide first whether the patch is in air only or in liquid open to the air. If it is open to the atmosphere, keep p_atm visible and then add the liquid part ρgh.",
                "If a liquid patch is open to the air, what two contributions must you look for before calculating total pressure?",
                "Do not treat ρgh as automatically the total pressure in every liquid problem. Open surfaces still have the atmosphere above them.",
...
M4_SPEC = {
    ...
    "mastery_outcomes": [
        ...
        "Use p_total = p_atm + ρgh in mixed pressure missions.",
    ],
    "lessons": [
        lesson_l1(),
        lesson_l2(),
