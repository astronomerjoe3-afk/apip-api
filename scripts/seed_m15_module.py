...
def lesson_one_quality_pass(lesson: Dict[str, Any]) -> Dict[str, Any]:
    diagnostic_extra = build_mcqs([
        ...
        ("M15L1_D8", "Why is brightness by itself too weak to prove something is a star?", ["Because a planet can look bright by reflecting starlight, so the stronger rule is whether the object makes its own light.", "Because apparent brightness can come from reflection, while stars are identified by self-produced light."], "Use source-of-light language.", ["star_reflects_light_confusion"], ["brightness_trap", "classify_light_source"], acceptance_groups(["brightness", "bright"], ["not enough", "too weak"], ["reflect", "reflection"], ["star", "self-lit", "makes its own light"])) ,
...
def lesson_two_quality_pass(lesson: Dict[str, Any]) -> Dict[str, Any]:
    diagnostic_extra = build_mcqs([
        ...
        ("M15L2_D9", "Why is a supernova not the ending for every star?", ["Because only sufficiently massive stars follow the supernova branch, while lower-mass stars can end as white dwarfs.", "Because stellar mass changes the later path, so not all stars reach the explosive route."], "Use mass-dependent branch language.", ["mass_same_star_fate_confusion"], ["supernova_path", "mass_life_path"], acceptance_groups(["mass"], ["not all", "only massive", "depends"], ["supernova"], ["white dwarf", "different ending", "different remnant"])) ,
