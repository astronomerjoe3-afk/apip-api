    ...
    return {
        ...
        "contract": contract(
            ...
            worked_examples=[
                ...
                worked("A particle diagram for liquid water shows particles close together but able to move past neighboring particles. Why does that identify a liquid rather than a solid or a gas?", ["Check spacing first: the particles are still close together, so the sample is not a gas.", "Then check mobility: the particles can change neighbors, so they are not locked into fixed positions like a solid.", "Match those two particle-model clues together to the liquid state."], "It identifies a liquid because the particles stay close together but can move past one another.", "Liquids keep particles close like solids, but unlike solids they allow neighbor changes; gases are ruled out because their particles are far apart.", "This keeps state identification tied to particle spacing and particle motion together, which is the actual physics test in the particle model."),
            ],
