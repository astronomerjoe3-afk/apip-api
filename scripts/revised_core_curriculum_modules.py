...
def _f5_l2_extras(lesson_tag: str, skills: Sequence[str]) -> Dict[str, List[Dict[str, Any]]]:
    return {
        "diagnostic": [
            ...
            short(
                f"{lesson_tag}_D10",
                "Why can sunrise happen without Earth completing a whole orbit around the Sun?",
                [
                    "because Earth only needs to rotate until the place faces sunlight",
                    "because spin brings the place into the lit half",
                    "because rotation not yearly orbit causes day and night",
                    "because Earth spins on its axis so sunrise does not need a full orbit",
                    "because Earth rotates on its axis while orbit takes much longer",
                ],
                "Use spin language and make clear that sunrise does not need a full orbit.",
                skill_tags=[skills[1]],
