...
def lesson_spec(lesson_id: str, title: str, sim_meta: Dict[str, Any], diagnostic: Sequence[Dict[str, Any]], analogy_text: str, commitment_prompt: str, micro_prompts: Sequence[Dict[str, str]], inquiry: Sequence[Dict[str, str]], recon_prompts: Sequence[str], capsule_prompt: str, capsule_checks: Sequence[Dict[str, Any]], transfer: Sequence[Dict[str, Any]], contract_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return enrich_m8_lesson(lesson)


def enrich_m8_lesson(lesson: Dict[str, Any]) -> Dict[str, Any]:
    lesson_id = str(lesson.get("id") or "")

    if lesson_id == "M8_L1":
        lesson["contract"]["visual_assets"] = m8_l1_optics_visuals()
        lesson["diagnostic"].extend(
            [
                mcq("M8L1_D9", "Object 7 cm in front of a plane mirror. Object-image separation?", ["14 cm", "7 cm", "3.5 cm", "21 cm"], 0, "Add the object distance to the equal image distance behind the mirror.", ["mirror_image_surface_confusion"], skill_tags=["mirror_image_position"]),
                short("M8L1_D10", "A route is 65 degrees to the mirror surface. What is the reflected angle to the Guide Line?", ["25 degrees", "25"], "Convert the surface angle to the Guide-Line angle first, then apply the equal-angle rule.", ["angle_from_surface_confusion", "reflection_equal_angle_confusion"], skill_tags=["surface_to_normal", "angle_calculation"], acceptance_rules=acceptance_groups(["25", "25 degrees"])),
            ]
        )
        lesson["capsule_checks"].extend(
            [
                short("M8L1_C7", "Why does a route along the Guide Line retrace its own path after reflection?", ["Because the incident angle is zero to the Guide Line, so the reflected angle is also zero and the route comes back on the same line.", "Because along the normal there is no sideways angle to reverse, so the reflected route retraces the same path."], "Use the zero-angle case around the normal.", ["reflection_equal_angle_confusion", "normal_reference_confusion"], skill_tags=["special_case_reflection"], acceptance_rules=acceptance_groups(["zero", "no"], ["guide line", "normal"], ["same line", "retraces", "comes back"])),
                mcq("M8L1_C8", "If the angle to the new Guide Line is kept at 30 degrees after the mirror tilts, the reflected angle is...", ["30 degrees", "60 degrees", "15 degrees", "90 degrees"], 0, "Reflection still compares equal angles to the current Guide Line.", ["normal_reference_confusion", "reflection_equal_angle_confusion"], skill_tags=["equal_angles"]),
            ]
        )
        lesson["transfer"].extend(
            [
                mcq("M8L1_M9", "Object 3 cm in front of a plane mirror. Image distance behind the mirror?", ["3 cm", "6 cm", "1.5 cm", "0 cm"], 0, "Plane-mirror images form the same distance behind the mirror as the object is in front.", ["mirror_image_surface_confusion"], skill_tags=["mirror_image_position"]),
                short("M8L1_M10", "Why can a screen not catch the image behind a plane mirror?", ["No real reflected routes meet behind the mirror; the image is only an apparent position from backward extensions.", "The mirror image is virtual, so a screen cannot intercept real light there.", "The image is virtual, so a screen cannot catch it.", "The image is virtual."], "Tie screenability to real-route crossings.", ["real_virtual_image_confusion", "ray_diagram_literal_confusion"], skill_tags=["real_vs_virtual"], acceptance_rules=acceptance_groups(["screen", "catch", "intercept"], ["cannot", "not", "no"], ["virtual", "apparent", "extension", "real", "actual"], ["meet", "cross", "image"])),
            ]
        )
        return lesson

    if lesson_id == "M8_L2":
        lesson["contract"]["visual_assets"] = m8_l2_visuals()
        lesson["diagnostic"].extend(
            [
                mcq("M8L2_D9", "A route enters a slower medium at 50 degrees to the Guide Line. The refracted angle should be...", ["less than 50 degrees", "greater than 50 degrees", "exactly 50 degrees", "90 degrees"], 0, "Toward the Guide Line means the refracted angle becomes smaller.", ["toward_away_normal_confusion"], skill_tags=["toward_normal_rule", "angle_comparison"]),
                short("M8L2_D10", "When light crosses a boundary, which usually stays fixed: frequency or speed?", ["Frequency stays the same.", "The frequency stays fixed while the speed changes."], "Keep source-set frequency separate from boundary-set speed.", ["refraction_speed_change_confusion"], skill_tags=["frequency_in_refraction"], acceptance_rules=acceptance_groups(["frequency"], ["same", "stays", "fixed"])),
            ]
        )
        lesson["capsule_checks"].extend(
            [
                mcq("M8L2_C7", "The strongest explanation for why a lens bends light is...", ["light refracts at the entry surface and again at the exit surface", "the lens middle pulls every route inward", "the lens stores the image", "the route reflects twice inside the lens"], 0, "A lens is best treated as two refracting boundaries.", ["lens_middle_pull_confusion"], skill_tags=["double_refraction"]),
                short("M8L2_C8", "Why does leaving glass for air make the route bend away from the Guide Line?", ["Because light moves into a faster medium, so the refracted angle to the Guide Line becomes larger.", "Because the new medium is faster than glass, so the route turns away from the normal."], "Use faster-medium language and the Guide Line together.", ["toward_away_normal_confusion", "refraction_speed_change_confusion"], skill_tags=["away_normal_rule"], acceptance_rules=acceptance_groups(["faster", "speed"], ["away", "larger"], ["guide line", "normal"])),
            ]
        )
        lesson["transfer"].extend(
            [
                short("M8L2_M9", "A route enters a slower medium at 60 degrees to the Guide Line. Should the refracted angle be greater or less than 60 degrees, and why?", ["Less than 60 degrees, because a slower medium bends the route toward the Guide Line.", "It should be smaller than 60 degrees because entering a slower medium turns the route toward the normal."], "State both the comparison and the reason.", ["toward_away_normal_confusion", "refraction_speed_change_confusion"], skill_tags=["toward_normal_rule", "speed_change_reasoning"], acceptance_rules=acceptance_groups(["less", "smaller"], ["60"], ["slower"], ["toward", "guide line", "normal"])),
                mcq("M8L2_M10", "Which statement best links a flat boundary to a lens?", ["A lens works by combining refraction at two boundaries.", "A lens is just a curved mirror.", "A lens only changes the brightness of light.", "A lens cannot be explained by boundary refraction."], 0, "The boundary story scales up to lenses.", ["lens_middle_pull_confusion", "lens_ray_rule_confusion"], skill_tags=["double_refraction"]),
            ]
        )
        return lesson

    if lesson_id == "M8_L3":
        lesson["contract"]["visual_assets"] = m8_l3_visuals()
        lesson["diagnostic"].extend(
            [
                mcq("M8L3_D9", "Object between F and 2F for a converging lens. Image?", ["Beyond 2F, real and larger", "Between lens and F, virtual and smaller", "At the lens center only", "Behind the object, virtual and larger"], 0, "Between F and 2F gives a real image beyond 2F and magnified.", ["lens_ray_rule_confusion", "real_virtual_image_confusion"], skill_tags=["image_region_reasoning"]),
                short("M8L3_D10", "Which route through a converging lens is treated as undeviated in the thin-lens model?", ["The center route.", "The ray through the optical center."], "Use the central-ray rule.", ["lens_ray_rule_confusion"], skill_tags=["central_ray_rule"], acceptance_rules=acceptance_groups(["center", "optical center", "middle"], ["route", "ray"])),
            ]
        )
        lesson["capsule_checks"].extend(
            [
                short("M8L3_C7", "Why can two selected routes be enough to locate a converging-lens image?", ["Because rule-based selected routes meet at the same image point as the full bundle, so they predict the image efficiently.", "Because a few principal rays follow reliable rules and still locate the same real image."], "Route sketches are efficient prediction tools.", ["ray_diagram_literal_confusion", "lens_ray_rule_confusion"], skill_tags=["ray_selection"], acceptance_rules=acceptance_groups(["few", "selected", "principal"], ["same", "image point", "image"], ["predict", "locate"], ["full bundle", "all rays", "efficient"])),
                mcq("M8L3_C8", "Object at 2F in front of a converging lens. Image?", ["At 2F on the far side, real and same size", "At F on the far side, real and smaller", "Between lens and F, virtual and larger", "At the lens center"], 0, "The 2F anchor case gives a same-size real image at 2F on the far side.", ["lens_ray_rule_confusion"], skill_tags=["image_region_reasoning"]),
            ]
        )
        lesson["transfer"].extend(
            [
                mcq("M8L3_M9", "Object between F and 2F for a converging lens. Which image description is best?", ["Real, beyond 2F, and larger", "Virtual, between lens and F, and smaller", "Real, at the lens center, and same size", "Virtual, behind the object, and smaller"], 0, "Between F and 2F makes the real image larger and farther out.", ["lens_ray_rule_confusion", "real_virtual_image_confusion"], skill_tags=["image_region_reasoning"]),
                short("M8L3_M10", "Why is the image from a converging lens called a True Meeting Point?", ["Because the actual refracted routes really cross there.", "Because real light rays meet there, so the image can be formed on a screen."], "Use actual-crossing language.", ["real_virtual_image_confusion"], skill_tags=["real_image_definition"], acceptance_rules=acceptance_groups(["actual", "real"], ["routes", "rays"], ["meet", "cross"], ["screen", "true meeting point"])),
            ]
        )
        return lesson

    if lesson_id == "M8_L4":
        lesson["contract"]["visual_assets"] = m8_l4_visuals()
        lesson["diagnostic"].extend(
            [
                mcq("M8L4_D9", "A route parallel to the axis reaches a diverging lens. After the lens it...", ["appears to come from the near focus on the object side", "passes through the far focus", "reflects straight back", "forms a real image at 2F"], 0, "Use the diverging parallel-ray rule.", ["lens_ray_rule_confusion"], skill_tags=["diverging_parallel_ray"]),
                short("M8L4_D10", "What image is most typical for a diverging lens with a real object?", ["A virtual, upright, smaller image.", "The image is virtual, upright, and reduced."], "State all three usual properties.", ["real_virtual_image_confusion"], skill_tags=["diverging_image_properties"], acceptance_rules=acceptance_groups(["virtual", "ghost"], ["upright"], ["smaller", "reduced"])),
            ]
        )
        lesson["capsule_checks"].extend(
            [
                mcq("M8L4_C7", "Where does the usual diverging-lens image appear?", ["Between the lens and the near focus on the object side", "Beyond 2F on the far side", "Exactly at the lens center", "At the far focus on the far side"], 0, "The virtual image stays on the object side between the lens and the near focus.", ["real_virtual_image_confusion", "lens_ray_rule_confusion"], skill_tags=["diverging_image_region"]),
                short("M8L4_C8", "Why can a screen not usually capture the image from a diverging lens?", ["Because the real routes do not meet; only backward extensions appear to meet.", "Because the image is virtual, so no actual rays cross at the image position."], "Tie the screen test to actual crossings.", ["real_virtual_image_confusion", "ray_diagram_literal_confusion"], skill_tags=["real_vs_virtual"], acceptance_rules=acceptance_groups(["screen"], ["not", "cannot"], ["real", "actual"], ["routes", "rays"], ["meet", "cross"], ["extension", "virtual"])),
            ]
        )
        lesson["transfer"].extend(
            [
                short("M8L4_M9", "After a diverging lens, what happens to the real routes, and what construction should you draw next to locate the image?", ["The real routes spread out, so you draw dashed backward extensions to find the ghost image.", "The outgoing rays diverge, then backward extensions are added to locate the virtual image."], "Keep the order real routes first, extensions second.", ["ray_diagram_literal_confusion", "real_virtual_image_confusion"], skill_tags=["virtual_extension"], acceptance_rules=acceptance_groups(["spread", "diverge"], ["dashed", "backward", "extension"], ["locate", "find"], ["virtual", "ghost"])),
                mcq("M8L4_M10", "If the object is moved but the lens stays diverging, the usual image remains...", ["virtual, upright, and on the object side", "real, inverted, and on the far side", "on the mirror surface", "a total-internal-reflection image"], 0, "Those are the usual diverging-lens image properties.", ["real_virtual_image_confusion"], skill_tags=["diverging_image_properties"]),
            ]
        )
        return lesson

    if lesson_id == "M8_L5":
        lesson["contract"]["visual_assets"] = m8_l5_visuals()
        lesson["diagnostic"].extend(
            [
                mcq("M8L5_D9", "Critical angle 42 degrees, incident angle 42 degrees. Boundary outcome?", ["The refracted route skims along the boundary", "Total internal reflection", "No bending at all", "The route enters the slower medium"], 0, "At the critical angle the refracted route runs along the boundary.", ["critical_angle_confusion"], skill_tags=["critical_angle_geometry"]),
                short("M8L5_D10", "State the two conditions needed for total internal reflection.", ["The light must try to leave the slower medium, and the incident angle must be greater than the critical angle.", "TIR needs light going from slower to faster and the incident angle above the critical angle."], "Give both the medium-direction condition and the angle condition.", ["critical_angle_confusion", "tir_direction_confusion"], skill_tags=["tir_conditions"], acceptance_rules=acceptance_groups(["leave", "slower", "higher index"], ["greater", "above"], ["critical angle"])),
            ]
        )
        lesson["capsule_checks"].extend(
            [
                short("M8L5_C7", "Why does 50 degrees in air-to-glass not count as total internal reflection even if 50 is larger than the critical angle from glass-to-air?", ["Because the route is entering the slower medium instead of leaving it, so the TIR direction condition is not met.", "Because total internal reflection only works when light tries to leave the slower medium; air to glass is the wrong direction."], "The number alone is not enough without the boundary direction.", ["tir_direction_confusion", "critical_angle_confusion"], skill_tags=["direction_condition"], acceptance_rules=acceptance_groups(["wrong direction", "entering", "not leaving"], ["slower medium", "glass"], ["tir", "total internal reflection"])),
                mcq("M8L5_C8", "Why does light stay trapped in an optical fiber core?", ["It keeps meeting the core-cladding boundary above the critical angle while trying to stay in the slower core", "The fiber core is hollow", "The light becomes a mirror", "The light stops moving"], 0, "Optical fibers rely on repeated TIR inside the core.", ["tir_direction_confusion", "critical_angle_confusion"], skill_tags=["application_link"]),
            ]
        )
        lesson["transfer"].extend(
            [
                mcq("M8L5_M9", "Critical angle 40 degrees, incident angle 36 degrees inside glass. What happens?", ["The route still refracts out", "The route must total-internally reflect", "The route stays exactly on the boundary", "The route becomes a mirror image"], 0, "Below the critical angle there is still an escaping refracted route.", ["critical_angle_confusion"], skill_tags=["critical_angle_geometry"]),
                short("M8L5_M10", "Critical angle 40 degrees, incident angle 55 degrees inside glass. What happens and why?", ["Total internal reflection occurs because the route is leaving the slower medium and the incident angle is above the critical angle.", "The route lock-bounces because it is in the correct glass-to-air direction and 55 degrees is greater than 40 degrees."], "Give both conditions in the explanation.", ["critical_angle_confusion", "tir_direction_confusion"], skill_tags=["tir_conditions", "direction_condition"], acceptance_rules=acceptance_groups(["total internal reflection", "lock-bounce"], ["leaving", "inside glass", "slower medium"], ["above", "greater"], ["critical angle"])),
            ]
        )
        return lesson

    if lesson_id == "M8_L6":
        lesson["contract"]["visual_assets"] = m8_l6_visuals()
        lesson["diagnostic"].extend(
            [
                mcq("M8L6_D9", "Which image could a screen capture?", ["A converging-lens image where actual routes meet", "A plane-mirror ghost image", "A diverging-lens ghost image", "A dashed extension"], 0, "A screen only catches a real image made by actual route crossings.", ["real_virtual_image_confusion"], skill_tags=["real_image_use"]),
                short("M8L6_D10", "What does a dashed line usually mean in an optics route sketch?", ["A backward extension showing where light appears to come from or where a virtual image is located.", "A construction line, not a real light path."], "Keep dashed lines separate from real routes.", ["ray_diagram_literal_confusion"], skill_tags=["extension_status"], acceptance_rules=acceptance_groups(["backward", "extension", "construction", "dashed"], ["not real", "apparent", "virtual"])),
            ]
        )
        lesson["capsule_checks"].extend(
            [
                mcq("M8L6_C7", "Which line is usually a reference rather than a light path?", ["the Guide Line", "the incident route", "the reflected route", "the refracted route"], 0, "The Guide Line is a reference for angle reading.", ["normal_reference_confusion", "ray_diagram_literal_confusion"], skill_tags=["diagram_roles"]),
                short("M8L6_C8", "Why can two different route sketches both be valid even if one uses dashed extensions and the other does not?", ["Because route sketches mix different line roles depending on whether the image is real or virtual, but both still predict image position correctly.", "Because a real-image sketch can use actual crossings while a ghost-image sketch uses extensions, and both are valid prediction tools."], "Use line-role language rather than saying one sketch is fake.", ["ray_diagram_literal_confusion", "real_virtual_image_confusion"], skill_tags=["diagram_purpose"], acceptance_rules=acceptance_groups(["different", "both valid"], ["real", "virtual", "ghost"], ["extension", "crossing", "line roles"], ["predict", "image"])),
            ]
        )
        lesson["transfer"].extend(
            [
                short("M8L6_M9", "Why is the screen test useful in route-sketch questions?", ["Because it checks whether actual light routes meet at the image point, which distinguishes real images from ghost images.", "Because a screen can catch a true meeting point but not a virtual image made only by extensions."], "Tie the test to actual crossings.", ["real_virtual_image_confusion"], skill_tags=["real_image_use"], acceptance_rules=acceptance_groups(["screen"], ["actual", "real"], ["meet", "cross"], ["distinguish", "separate"], ["virtual", "ghost", "real"])),
                mcq("M8L6_M10", "Which case should include dashed backward extensions to locate the image?", ["a diverging-lens ghost image", "a converging-lens real image on a screen", "a route along the Guide Line only", "a current in a circuit"], 0, "Dashed backward extensions belong to virtual-image cases.", ["ray_diagram_literal_confusion", "real_virtual_image_confusion"], skill_tags=["extension_status"]),
            ]
        )
        return lesson
...
def mirror_lesson() -> Dict[str, Any]:
    d = [
        ...
        short("M8L1_D5", "A route is 20 degrees to the mirror surface. Angle to the Guide Line?", ["70 degrees", "70"], "Surface and normal differ by 90 degrees.", ["angle_from_surface_confusion"], skill_tags=["surface_to_normal"], acceptance_rules=acceptance_groups(["70", "70 degrees"])),
    ...
    t = [
        ...
        short("M8L1_M2", "15 degrees to the surface equals what incident angle to the Guide Line?", ["75 degrees", "75"], "Use 90 - 15.", ["angle_from_surface_confusion"], skill_tags=["surface_to_normal"], acceptance_rules=acceptance_groups(["75", "75 degrees"])),
        ...
        short("M8L1_M4", "Why is the mirror image a Ghost Meeting Point?", ["The reflected routes only appear to meet when extended backward.", "The image is found from apparent backward extensions rather than real crossings.", "The image is virtual, so it is only an apparent meeting point."], "Use apparent-meeting language.", ["real_virtual_image_confusion", "ray_diagram_literal_confusion"], skill_tags=["real_vs_virtual"], acceptance_rules=acceptance_groups(["appear", "apparent", "virtual", "ghost"], ["meet", "come from", "meeting point"], ["backward", "extension", "not real", "not actual"])),
        ...
        short("M8L1_M7", "Why does the mirror image stay the same distance behind the surface?", ["The equal-angle geometry makes the backward extensions meet at the symmetric point behind the mirror.", "The reflected routes are symmetric about the Guide Line, so the apparent image is equally far behind.", "Plane-mirror geometry is symmetric, so the image stays the same distance behind the mirror."], "Use symmetry language.", ["mirror_image_surface_confusion"], skill_tags=["mirror_image_position"], acceptance_rules=acceptance_groups(["symmetric", "symmetry", "same distance", "equally far"], ["behind"], ["guide line", "mirror", "plane mirror", "reflection"])),
...
def refraction_lesson() -> Dict[str, Any]:
    ...
    return lesson_spec(
        "M8_L2",
        "Bend Gates, Fast Zones, and Slow Zones",
        sim(
            "m8_bend_gate_lab",
            "Bend Gate lab",
            "Compare fast-to-slow and slow-to-fast boundaries.",
            ["Start with air to glass.", "Reverse to glass to air.", "Keep the Guide Line visible both times."],
            ["Explain refraction as a speed-change effect.", "Predict toward-normal and away-normal bends.", "Describe a lens as two refractions."],
            ["incident_angle_deg", "speed_ratio", "refracted_angle_deg", "boundary_case"],
            "Speed-zone boundary reasoning.",
        ),
        d,
        "A Glow-Route turns at a Bend Gate because the new zone changes how fast light travels.",
        "Before calling the bend direction, decide whether the next zone is slower or faster.",
        [prompt_block("If the next zone is slower, which way does the route turn?", "Toward the Guide Line."), prompt_block("Why is the lens-middle-pull story weak?", "The turn happens at the boundaries.")],
        [prompt_block("Compare air to glass with glass to air.", "One turns toward the Guide Line; the other turns away."), prompt_block("Hold the incident angle steady while you change the speed ratio.", "The bend follows the medium change.")],
        ["Explain the cause of refraction using speed-change language.", "Explain why a lens bends at entry and exit surfaces."],
        "Keep the speed-zone cause and the Guide Line reference visible before you judge the bend direction.",
        c,
        t,
        contract(
            concept_targets=["Explain refraction as a speed-change effect.", "Predict toward-normal and away-normal bends.", "Describe a lens as a pair of refracting boundaries."],
            core_concepts=["Refraction is a direction change caused by a speed change at a boundary.", "Entering a slower medium bends the route toward the normal.", "Entering a faster medium bends the route away from the normal.", "A lens bends routes at entry and exit surfaces rather than by pulling them toward the middle."],
            prerequisite_lessons=["M8_L1"],
            misconception_focus=["refraction_speed_change_confusion", "toward_away_normal_confusion", "normal_reference_confusion", "angle_from_surface_confusion", "lens_middle_pull_confusion", "lens_ray_rule_confusion"],
            formulas=[relation("slower medium -> toward the normal; faster medium -> away from the normal", "Refraction direction follows how the light speed changes across the boundary.", ["degrees"], "Use when comparing incident and refracted routes.")],
            representations=[representation("words", "Explains the speed-change reason."), representation("diagram", "Shows incident and refracted routes with the Guide Line."), representation("formula", "Compresses the toward-normal and away-normal rule.")],
            analogy_map=glow_route_map("the class is comparing fast zones, slow zones, and refracting boundaries"),
            worked_examples=[
                worked("A route goes from air into glass. Which way does it bend?", ["Identify air as faster and glass as slower.", "Use the speed-change rule.", "State the turn relative to the Guide Line."], "Toward the Guide Line", "Entering a slower medium bends the route toward the normal.", "This gives the basic bend direction a clear anchor."),
                worked("A route leaves glass and enters air. Which way does it bend?", ["Identify glass as slower and air as faster.", "Use the reverse speed-change rule.", "State the turn relative to the Guide Line."], "Away from the Guide Line", "Leaving a slower medium for a faster one bends the route away from the normal.", "This secures the reverse case."),
                worked("A student says the lens pulls the route toward the middle. Correct it.", ["Look for where the direction changes.", "Notice the entry and exit surfaces.", "State the cause as a medium change at each surface."], "The lens bends routes because light refracts at its two surfaces.", "A lens is two boundary turns, not a middle pull.", "This blocks a persistent lens misconception."),
            ],
            visual_assets=m8_l2_visuals(),
            animation_assets=[animation("m8-l2-zone-turn", "refraction", "Boundary turn", "Shows the route bend toward the Guide Line on entry to a slower zone and away on exit to a faster zone.")],
            ...
            reflection_prompts=["Why does a Glow-Route turn at a Bend Gate?", "Why is a lens better described as two boundary turns than a middle pull?"],
            mastery_skills=["refraction_classification", "toward_normal_rule", "away_normal_rule", "speed_change_reasoning", "double_refraction"],
            variation_plan={"diagnostic": "Fresh attempts rotate between fast-to-slow, slow-to-fast, and weak-explanation stems.", "concept_gate": "Concept checks vary between direction prediction and cause explanation.", "mastery": "Mastery mixes media pairs, Guide-Line reasoning, and lens-as-two-boundaries prompts before repeating stems."},
            scaffold_support=scaffold("Refraction is a boundary turn caused by a speed change in the new medium.", "Ask two questions in order: is the new medium slower or faster, and which line is the Guide Line?", "If light enters a slower medium, which way should the route turn relative to the Guide Line?", "Do not describe the bend as if the route is being pulled toward the lens middle.", "The Bend Gate keeps the route, the Guide Line, and the speed labels in one picture so the direction rule feels causal rather than magical.", "What changes first at the boundary: the route speed or the lens center?", [extra_section("Toward and away", "Toward the Guide Line means a smaller angle to the normal; away means a larger angle to the normal.", "Which phrase belongs to a route entering a slower zone?"), extra_section("Two bends in a lens", "A thin lens can be treated as two refracting surfaces, so the route changes direction at entry and then again at exit.", "Why is a lens more than one single bend?")]),
...
_sync_assessed_skills(M8_LESSONS)
